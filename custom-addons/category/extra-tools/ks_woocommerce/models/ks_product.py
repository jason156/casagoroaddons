...
class KsProduct(models.Model):
    ...
    def ks_prepare_product_variant_data_to_export_in_woo(self, product_record):
        ...
        if product_record.ks_woo_variant_id == 0:
            ...
            price_list = product_record.ks_woo_instance_id.ks_woo_pricelist
            prices = self.ks_price_export_in_woo(price_list, product_record)
            data.update(prices)
        return data
    ...
    #
    # def ks_manage_woo_image(self, json_data, instance_id, product_template):
    #     if json_data:
    #         images_ids = [each_img['id'] for each_img in json_data]
    #         product_template.ks_product_image_ids.search([('ks_woo_id', 'not in', images_ids),
    #                                               ('ks_woo_instance_id', '=', instance_id.id),
    #                                               ('ks_product_tmpl_id', '=', product_template.id)]).unlink()
    ...
    def ks_update_price_on_product(self, json_data):
        ...
        if self.ks_woo_instance_id.ks_woo_pricelist and self.ks_woo_product_type == 'simple':
            self.ks_update_price_on_price_list(json_data, self.ks_woo_instance_id.ks_woo_pricelist,
    ...
    def ks_manage_inventory_adjustments(self, product_records, instance_id, wcapi):
        ...
        if record:
            inventory_adjustment = self.env['stock.inventory'].create({
                'name': instance_id.ks_name + ' [' + str(fields.datetime.now()) + ']',
                'company_id': instance_id.ks_company.id,
