    ...
    def create(self,vals):
        ...
        if new_id.order_line:
            ...
            if suggested_products:
                ...
                template_customer_id = self.env.ref('similar_products_suggestion.email_template_edi_suggested_product_customer')
                template_user_id = self.env.ref('similar_products_suggestion.email_template_edi_suggested_product_user')
                for sol in new_id.order_line :
                    if sol.product_id.qty_available <= 0:
                        send = template_customer_id.send_mail(sol.id, force_send=True)
                        send = template_user_id.send_mail(sol.id, force_send=True)
        return new_id
